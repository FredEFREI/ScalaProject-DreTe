...
class TestSuiteNonDirectedWeighted extends AnyFlatSpec {
  ...
  it should "be initialized" in {
    val graph = GraphNonDirectedWeighted[Int](List(), List())
    assert(graph == GraphNonDirectedWeighted[Int](List(), List()))
    assert(graph != null)
  }

  it should "get the list of nodes" in {
    val graph = GraphNonDirectedWeighted[Int](List(2, 6, 7), List())
    assert(graph.getNodes == List(2, 6, 7))
    val graph1 = GraphNonDirectedWeighted(List(), List())
  ...
  it should "add an Node" in {
    ...
    assert(graph_node == GraphNonDirectedWeighted[Int](List(2), List()))
    assert(graph_node.addNode(2) == GraphNonDirectedWeighted[Int](List(2), List()))
  ...
  it should "remove an Node" in {
    ...
    assert(graph_node == GraphNonDirectedWeighted[Int](List(6, 7), List()))
  ...
  it should "add an Edge" in {
    ...
    assert(graph_edge == GraphNonDirectedWeighted[Int](List(2, 4), List(((2, 4), 6))))
  ...
  it should "remove all edges linked to a removed node" in {
    val graph = GraphNonDirectedWeighted[Int](List(2, 6, 7), List(((2, 6), 6)))
    val graph_node = graph.removeNode(2)
    assert(graph_node == GraphNonDirectedWeighted[Int](List(6, 7), List()))
    assert(graph_node.getNodes == List(6, 7))
  ...
  it should "be able to get all nodes that can be accessed from a selected node" in {
    val graph = GraphNonDirectedWeighted(List(1, 2, 3, 5, 4, 10, 6, 7), List(((1, 2), 1), ((3, 2), 3), ((1, 3), 4), ((4, 3), 8), ((4, 5), 5), ((5, 2), 2), ((1, 6), 9)))
  ...
  it should "transform into a 'Dot' format" in {
    val graph = GraphNonDirectedWeighted(List(1, 2, 3, 5, 4, 10, 6, 7),List(((1,2),1), ((3,2),3), ((1,3),4), ((4,3),8), ((4,5),5), ((5,2),2), ((1,6),9)))
    assert(graph.toDot == "graph { 1 2 3 5 4 10 6 7 1 -- 2 [label=1] 3 -- 2 [label=3] 1 -- 3 [label=4] 4 -- 3 [label=8] 4 -- 5 [label=5] 5 -- 2 [label=2] 1 -- 6 [label=9] }")
    val graph2 = GraphNonDirectedWeighted(List(), List())
  ...
  it should "compute the dfs algorithm" in {
    ...
    val graph2 = GraphNonDirectedWeighted(List(1), List())
  ...
  it should "compute the bfs algorithm" in {
    ...
    val graph1 = GraphNonDirectedWeighted(List(1), List())
  ...
  it should "detect if a graph has a cycle in it" in {
    ...
    assert(!GraphNonDirectedWeighted(List(1, 2, 3, 4, 5, 6),List(((1, 2), 1),((3, 4), 1),((5, 6), 1))).cycleDetection())
  }
