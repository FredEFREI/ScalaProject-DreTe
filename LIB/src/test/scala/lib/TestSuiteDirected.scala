...
class TestSuiteDirected extends AnyFlatSpec {
  behavior of "A Directed Weighted Graph"

  it should "be initialized" in {
    val graph = GraphDirected[Int](List(), List())
    assert(graph == GraphDirected[Int](List(), List()))
    assert(graph != null)
  }

  it should "get the list of nodes" in {
    val graph = GraphDirected[Int](List(2, 6, 7), List())
    assert(graph.getNodes == List(2, 6, 7))
    val graph1 = GraphDirected(List(), List())
  ...
  it should "add an Node" in {
    ...
    assert(graph_node == GraphDirected[Int](List(2), List()))
    assert(graph_node.addNode(2) == GraphDirected[Int](List(2), List()))
  ...
  it should "remove an Node" in {
    ...
    assert(graph_node == GraphDirected[Int](List(6, 7), List()))
    assert(graph_node.getNodes == List(6, 7))
  }

  it should "get the list of Edges" in {
    val graph = GraphDirected[Int](List(2, 4), List((2, 4)))
  ...
  it should "add an Edge" in {
    ...
    assert(graph_edge == GraphDirected[Int](List(2, 4), List((2, 4))))
  ...
  it should "remove an Edge" in {
    ...
    assert(graph_edge == GraphDirected[Int](List(2, 4), List()))
  }

  it should "remove all edges linked to a removed node" in {
    val graph = GraphDirected[Int](List(2, 6, 7), List((2, 6)))
    val graph_node = graph.removeNode(2)
    assert(graph_node == GraphDirected[Int](List(6, 7), List()))
    assert(graph_node.getNodes == List(6, 7))
  ...
  it should "be able to get all nodes that can be accessed from a selected node" in {
    val graph = GraphDirected(List(1, 2, 3, 5, 4, 10, 6, 7), List((1, 2), (3, 2), (1, 3), (4, 3), (4, 5), (5, 2), (1, 6)))
  ...
  it should "transform into a 'Dot' format" in {
    val graph = GraphDirected(List(1, 2, 3, 5, 4, 10, 6, 7),List((1, 2), (3, 2), (1, 3), (4, 3), (4, 5), (5, 2), (1, 6)))
    assert(graph.toDot == "digraph { 1 2 3 5 4 10 6 7 1 -> 2 3 -> 2 1 -> 3 4 -> 3 4 -> 5 5 -> 2 1 -> 6 }")
    val graph2 = GraphDirected(List(), List())
  ...
  it should "compute the dfs algorithm" in {
    ...
    val graph2 = GraphDirected(List(1), List())
  ...
  it should "compute the bfs algorithm" in {
    ...
    val graph1 = GraphDirected(List(1), List())
